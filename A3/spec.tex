...
\subsubsection* {Exported Types}
 ...
 T \wss{What should be written here?}
...
$S$: List of T\wss{What is the type of the state variable?}
...
cnt\_cards($T$, $F$, $D$, $W$, 104 \wss{\text{What goes here?}}) = TOTAL\_CARDS\\
...
\begin{tabular}{|p{3cm}|l|}
\hhline{|-|-|}
$c = \mbox{Tableau}$ & T[$n_0$], T[$n_1$] := T[n\_0].pop(),T[n\_1].push(T[n\_0].top()) \wss{What goes here?}\\
\hhline{|-|-|}
$c = \mbox{Foundation}$ & T[$n_0$], F[$n_1$] := T[n\_0].pop(),F[n\_1].push(T[n\_0].top())\wss{What goes here?}\\
\hhline{|-|-|}
\end{tabular}
