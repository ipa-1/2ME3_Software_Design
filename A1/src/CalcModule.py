...
# Assumption: All free choice students will be granted their first choice
# regardless of capacity


def allocate(S, F, C):
    allocated = {'civil': [], 'chemical': [], 'electrical': [],
    ...
    # Allocate the students with free choice by finding their record in the
    # general list, checking gpa, and capacity of desired choice
        ...
        for all_student in allocating:  # Iterate through all students in general list
            if (all_student)['macid'] == fc_student:  # If macid matches
                if (all_student['gpa']) > 4:  # Check if their gpa entry is greater than 4
                    ...
                    allocated[student_choice].append(
                        (all_student))  # Add student to department
                    # Remove student from general list to keep track of who has
    ...
    # Allocate the remaining students, the list is already sorted and in order
    # of gpa
